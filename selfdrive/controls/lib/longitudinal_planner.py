...
from selfdrive.controls.lib.dynamic_longitudinal_controller import DynamicLongitudinalController
...
class LongitudinalPlanner:
  def __init__(self, CP, init_v=0.0, init_a=0.0):
    ...
    self.dynamic_longitudinal_controller = DynamicLongitudinalController()
  ...
  def update(self, sm):
    ...
    if self.param_counter % 50 == 0:
      self.dynamic_longitudinal_controller.read_params()
    self.param_counter += 1
    #self.mpc.mode = 'blended' if sm['controlsState'].experimentalMode else 'acc'
    self.mpc.mode = self.dynamic_longitudinal_controller.set_mpc_mode(self.mpc.mode, self.CP.radarUnavailable, sm['carState'], sm['radarState'].leadOne, sm['modelV2']) if sm['controlsState'].experimentalMode else 'acc'
